...
def _get_latest_version(pkgname):
    ...
    if pkgname in cfg.mpr_cache.package_bases: version_list += [cfg.mpr_cache.package_dicts[pkgname].version]
    ...
    for i in version_list:
        if check_version(i, ">", returned_version): returned_version = i
...
def _get_description(pkgname):
    try:
        ...
        if pkgname not in cfg.mpr_cache.package_names: return None
...
def search():
    ...
    for pkg in cfg.packages:
        ...
        if ("--apt-only" not in cfg.options) and (not cfg.config_data["search"]["apt_only"]):
            ...
            for mpr_pkg in cfg.mpr_cache.package_dicts:
                ...
                if pkgdesc is None: pkgdesc = ""

                if (pkg in mpr_pkg) or (pkg in pkgdesc): cfg.mpr_packages += [mpr_pkg]
    ...
    for index, pkgname in enumerate(packages):
        ...
        results_string += f"{colors.apt_green}{pkgname}{colors.normal}/{pkgver}{bracketed_text}\n"
        results_string += f"  {pkgdesc}\n"
        
        if index < list_length: results_string += "\n"

    if (len(results_string.splitlines()) > get_terminal_size().lines) and ("--skip-less-pipe" not in cfg.options):
