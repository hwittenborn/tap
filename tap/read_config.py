    ...
    for item in _required_keys:
        for key in _key_maps[item]:
            if key not in config_dict[item]:
                msg = message.warning2(f"Missing key '{key}' under toplevel item '{item}'.", value_return=True)
                bad_config_reasons += [msg]
                ...
                try:
                    ...
                    msg = message.warning2(f"Invalid value for '{key}' under toplevel item '{item}'.", value_return=True)
                    bad_config_reasons += [msg]
                    cfg.config_data[item][key] = _default_config[item][key]

    if bad_config_reasons != []:
        message.warning("Config file is invalid:")
        for i in bad_config_reasons: print(i, end="")
