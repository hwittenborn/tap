...
def _update():
    try:
        ...
        msg = message.error("Failed to download package cache from MPR.", newline=False, value_return=True)
        raise newline_error_exception(msg)
    
    except json.decoder.JSONDecodeError:
        msg = message.error("Error parsing returned package cache from MPR.", newline=False, value_return=True)
        raise newline_error_exception(msg)
